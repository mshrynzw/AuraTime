...
- **業務監視**: 
  - CloudWatch Alarms: 失敗した `t_job_runs` の検知
  - CloudWatch Logs Insights: `h_audit_logs`での不審な操作の検知
- **Amplify監視**: 
  - ビルドステータス（成功/失敗）
  - デプロイステータス
  - CloudFront配信状況（CDN）
...
### アラート通知
- **PWA通知**: CloudWatch Alarms → SNS → Lambda → バックエンドAPI → PWA Push Notification
  - System Adminロールのユーザーに対して、運用アラートをPWA通知で送信
  - 通知内容: ジョブ実行失敗、パフォーマンス低下、不審な操作検知など
  - 実装: `POST /api/v1/notifications/push` APIを呼び出し、PWAのPush Subscriptionを利用
- **PagerDuty連携**: 重大な障害時の自動エスカレーション（確実な通知が必要な場合）

### PWA通知の実装フロー
1. CloudWatch Alarmがトリガー
2. SNSトピックに通知
3. Lambda関数がSNSメッセージを受信
4. Lambda関数がバックエンドAPI (`POST /api/v1/notifications/push`) を呼び出し
5. バックエンドがSystem AdminロールのユーザーのPush Subscriptionを取得
6. Web Push APIでPWA通知を送信
7. ユーザーの端末でPWA通知が表示される

## 2. 障害対応フロー
1.  **検知**: CloudWatch Alarms、PWA通知、PagerDuty、またはユーザーからの申告
2.  **初期調査**: 
   - `request_id` を元に CloudWatch Logs Insights で検索
   - RDS Performance Insights でDBパフォーマンス確認
   - `t_job_runs` / `h_audit_logs` テーブルを直接確認
3.  **応急処置**: 
